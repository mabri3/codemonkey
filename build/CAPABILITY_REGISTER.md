...
| bestofn | PROVEN-LIVE | cycle-79 scripted 2-attempt real-exec run (first-pass wins, byte-identical reset) + `exec --best-of 2` → exit 2 (`build/probes/cycle79-cli.out`); loop48 C113: refine pass through the REAL exec path — two candidates fail, the refine is seeded with their bounded evidence (both `[candidate N]` blocks observed in the prompt the provider received), refined tree stands, `bestofn.refine {candidates:2, refined:1, verified:true}` on the trace, exit 0; candidate-1-pass makes ZERO extra calls; refine-failure stays honest; CLI `--refine-seeded` without `--best-of` → exit 2 with the reason (`build/probes/cycle113-probe.out`; `tests/test_bestofn_refine.py` 6 tests incl. the CLI pin); loop48 C114: tier-2 tournament — `exec --best-of 2 --tournament-compare <cmd>` through the CLI selected candidate 2 (winner's tree restored, stdout = winner's message, exit 0; malformed cmd → exit 1, last tree kept) (`build/probes/cycle114-probe.out`; `tests/test_bestofn_tournament.py` 10 tests incl. permutation determinism + honest empty without a comparison) |
